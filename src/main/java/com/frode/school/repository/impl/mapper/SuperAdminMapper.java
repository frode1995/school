...
public interface SuperAdminMapper {
    ...
    /*@Results(id="SuperAdmin",value = {
            @Result(property = "id", column = "id", javaType = Integer.class, jdbcType = JdbcType.INTEGER, id = true),
            @Result(property = "name", column = "name", javaType = String.class, jdbcType = JdbcType.VARCHAR),
            @Result(property = "password", column = "password", javaType = String.class, jdbcType = JdbcType.VARCHAR),
    })*/
    @Select("select * from teacher where id = #{id}")//从教师表中查询tecId
    SuperAdminPO  getSuperAdminPOById(@Param("id") Integer id);
    ...
    @Select("select * from teacher where id = #{id}")//从教师表中查询tecId
    SuperAdminPO  superAdminLogin(@Param("id") Integer id);//验证用户登录的方法
